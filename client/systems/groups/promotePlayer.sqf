...
if(player != leader group player) exitWith {player globalChat format["No eres líder por lo que no puedes ascender jugadores!"];};
...
//Checks
if (isNil "_target") exitWith { player globalChat "Primero selecciona el jugador." };

if (_target == player) exitWith { player globalChat "No puedes ascenderte a ti mismo." };
...
if (!_inCombat) then
{
	...
	player globalChat format ["Has ascendido a %1 y ahora es el líder del grupo", name _target];
	player setVariable ["currentGroupIsLeader", false, true];
	_target setVariable ["currentGroupIsLeader", true, true];
}
else
{
	player globalChat "El jugador está en combate. No puedes hacerle líder ahora mismo.";
};
