...
storePurchaseHandle = _this spawn
{
	...
	_showInsufficientFundsError =
	{
		...
		hint format ["No tienes suficiente dinero para ""%1""", _itemText];
	...
	{
		_itemText = _this select 0;
		hint format ["No tienes suficiente espacio para ""%1""", _itemText];
		playSound "FD_CP_Not_Clear_F";
		_price = -1;
	};
	...
	if (!isNil "_price" && {_price > -1}) then
	{
		...
		_playerMoneyText ctrlSetText format ["Dinero: $%1", [player getVariable ["cmoney", 0]] call fn_numbersText];
