...
if (_uid call isAdmin) then
{
...

	{
		...
		if(side _x == INDEPENDENT) then {_side = "Independientes";};
		_namestr = name(_x) + " [UID:" + getplayerUID(_x) + "] [Equipo:" + format["%1",_side] + "] [Equipo asignado:"+format["%1",_lockedSide]+"] [Castigado:" + format["%1",_punishCount]+ "]";
