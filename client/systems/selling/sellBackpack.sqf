...
if (isNull backpackContainer player) exitWith
{
	...
	hint "No tienes mochila para vender!";
};
...
storeSellingHandle = _this spawn
{
	...
	if ([parseText _confirmMsg, "Confirm", "Sell", true] call BIS_fnc_guiMessage) then
	{
		...
		if (CARGO_STRING(_obj) == _originalCargo) then
		{
			...
			hint format ['Has vendido "%1" por $%2', _objName, _sellValue];
			playSound "FD_Finish_F";
		}
		else
		{
			playSound "FD_CP_Not_Clear_F";
			[format ['El contenido de "%1" ha cambiado. Por favor, inicia de nuevo el proceso de venta.', _objName], "Error"] call BIS_fnc_guiMessage;
		};
	};
};
